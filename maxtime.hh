...
{
	// TODO: implement this function, then delete the return statement belows
	...
	for (size_t i = 1; i <= n; i++)
	{
		for (int j = 1; j <= total_cost; j++)
		{
			if (j >= rides[i-1]->cost())
			{
				if (T[i - 1][j] >= T[i - 1][j - rides[i-1]->cost()] + rides[i-1]->time())
				{
					T[i][j] = T[i - 1][j];
				}
				else
				{
					T[i][j] = T[i - 1][j - rides[i-1]->cost()] + rides[i-1]->time();
				}
			}
			else
			{
	...
	int col_shift = total_cost;
	for (int i = rides.size(); i > 0; i--)
	{
		if (T[i][col_shift] != T[i - 1][col_shift])
		{
			result->push_back(rides[i-1]);
			col_shift -= rides[i - 1]->cost();
